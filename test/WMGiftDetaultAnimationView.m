...
-(void)createUI{
    ...
    [self.labName mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(self.bgAllView.mas_centerX);
        make.centerY.equalTo(self.bgAllView.mas_centerY).offset(250);
    ...
    [self.bgImageView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(self.gifImageView);
        make.centerY.equalTo(self.gifImageView).offset(260);
...
-(void) showAnimation {
    ...
    moveAnimation.toValue = [NSValue valueWithCGPoint:CGPointMake([UIScreen mainScreen].bounds.size.width*0.5, CurrentHeight * 0.5 + 195)];
        moveAnimation.duration = 0.5+0.5;
...
//    moveAnimation.
    ...
    enlargeAnimation.beginTime = 0.4+0.5;
    ...
    narrowAnimation.beginTime = 0.9+0.5;
    ...
    bgImageViewTransformAnima.beginTime = 0.5+0.5 ;

    bgImageViewTransformAnima.duration = 2.5; // 持续时间
    bgImageViewTransformAnima.repeatCount = 8 ; // 重复次数
...
-(UIView *)bgAllView
{
    if (!_bgAllView) {
        ...
        _bgAllView.backgroundColor = [UIColor clearColor];
